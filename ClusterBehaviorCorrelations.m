...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i = firstdatainput:length(varargin) 
        ...
        if ~isempty(Correlations{session})
            ...
            dist_between_cue_spines = nan(1,size(allspine_combos,1));
            dist_between_mov_spines = nan(1,size(allspine_combos,1));
                corr_between_mov_spines = nan(1,size(allspine_combos,1));
            dist_between_presuc_spines = nan(1,size(allspine_combos,1));
            dist_between_suc_spines = nan(1,size(allspine_combos,1));
            dist_between_movduringcue_spines = nan(1,size(allspine_combos,1));
            dist_between_rew_spines = nan(1,size(allspine_combos,1));
            ...
            if sum(MovementSpines)>1
                ...
                for s = 1:length(spinelist)
                    ...
                    NearestMovSpine{session}(1,s) = nanmin(cell2mat(arrayfun(@(x) DistanceMap(currentspine,x), otherspines, 'uni', false)));   %%% Find the smallest distance from the current spine to other spines on the same dendrite
                end
            end
...
%             corrmat = corrcoef([varargin{i}.OverallSpineActivity]');
            ...
            spinedatatouse = varargin{i}.OverallSpineActivity;
            
            [r_sp_d, ~] = corrcoef([spinedatatouse', varargin{i}.Dendrite_Binarized']);
            
            corrmattouse = r_sp_d;
            causcorrmattouse = r_sp_d;
...
%     ylim([0 1.2])
    ...
    subplot(2,4,1)
        ...
        ydata = [CorrelationBetweenMovementSpines{1}, CorrelationBetweenMovementSpines{2}]';
        ydata(isnan(ydata)) = 0;
        plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', lgreen); hold on;
        plot(xdata(ydata<0.5), ydata(ydata<0.5), '.k')
...
%     %         plot(-1/decay.b,0.368*decay.a, 'ok', 'MarkerFaceColor', 'k') %%% 0.368 corresponds to the decay constant, tau
    ...
    subplot(2,4,currentplot)
        ...
        ydata = [CorrelationBetweenMovementSpines{10},CorrelationBetweenMovementSpines{11}]';
        ydata(isnan(ydata)) = 0;
        plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', lgreen); hold on;
        plot(xdata(ydata<0.5), ydata(ydata<0.5), '.k')
%             decay = fit(xdata, ydata, 'exp1'); 
...
%     %         plot(-1/decay.b,0.368*decay.a, 'ok', 'MarkerFaceColor', 'k') %%% 0.368 corresponds to the decay constant, tau
    ...
    subplot(2,4,currentplot)
        ...
        ydata = [CorrelationBetweenAllSpines{1}, CorrelationBetweenAllSpines{2}]';
        ydata(isnan(ydata)) = 0;
...
%             legend off
    ...
    subplot(2,4,currentplot)
        ...
        ydata = [CorrelationBetweenAllSpines{10}, CorrelationBetweenAllSpines{11}]';
        ydata(isnan(ydata)) = 0;
        plot(xdata(ydata>=0.5), ydata(ydata>=0.5), '.', 'Color', dred); hold on;
        plot(xdata(ydata<0.5), ydata(ydata<0.5), '.k')
            decay = fit(xdata, ydata, 'exp1'); 
...
%             legend off
    ...
    corrtouse{1} = [CorrelationBetweenAllSpines{1}, CorrelationBetweenAllSpines{2}];
    ...
    corrtouse{2} = [CorrelationBetweenAllSpines{10}, CorrelationBetweenAllSpines{11}];
    disttouse{2} = [AllDistancesBetweenAllSpines{10}, AllDistancesBetweenAllSpines{11}];

    corrtouse{3} = [CorrelationBetweenMovementSpines{1}, CorrelationBetweenMovementSpines{2}];
    disttouse{3} = [AllDistancesBetweenMovementSpines{1}, AllDistancesBetweenMovementSpines{2}];
    %         farcorrtouse = CorrelationBetweenFarSpines{sessiontouse};

    corrtouse{4} = [CorrelationBetweenMovementSpines{10}, CorrelationBetweenMovementSpines{11}];
    ...
    subplot(2,4,3); hold on;
        ...
        n = n/sum(n);
        bar(n, 'FaceColor', lpurple)
        ylim([0 1])
        legend({'Nearest MR spine'})
    ...
    subplot(2,4,4); hold on;
        ...
        ylim([0 1])
        legend({'All spine pairs', 'MR spines'})
        title('Session 1-2', 'Fontsize', 14)
        set(gca, 'XTick', [0:30], 'XTickLabel', mat2cell(num2str([0:5:150]')))
        xlabel('Distance bins')
