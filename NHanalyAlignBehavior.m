...
if ~isempty(extreme_overlaps)
    disp([num2str(length(extreme_overlaps)), ' trials are significantly overlapping! This is a big problem!'])
else
end
...
if imagingframestouse(end)>length(Fluor.SynapseOnlyBinarized)       %%% Occasionally, the behavior goes longer than the imaging, so cut off the frames to use variable at the last imaging frame
    stopping = find(imagingframestouse == length(Fluor.SynapseOnlyBinarized));
    if isempty(stopping)
        ...
        stopping = find(imagingframestouse == C(end));
    end
    imagingframestouse = imagingframestouse(1:stopping);
end

seek = 1; found = 0;
while found == 0        %%% Find the first integer value to use
    if ~~mod(imagingframestouse(seek),1)
        seek = seek+1;
    else
        found = 1;
        starterframe = seek;
...
if ~isempty(skippedframes)
    ...
    for s = 1:numberofSpines
        FramedProcessed_dFoF(s,1:length(imagingframestouse)) = Fluor.Processed_dFoF(s,imagingframestouse);
        Framed_dFoF(s,1:length(imagingframestouse)) = Fluor.dF_over_F{s}(imagingframestouse);
    end
...
signaltouse = cellfun(@(x) x(imagingframestouse), Fluor.dF_over_F, 'UniformOutput', false);
processed_signaltouse = spine_Data';
processed_dendsub_signaltouse = spine_Data_DendriteSubtracted';
...
for i = firsttrial:length(Behavior.Behavior_Frames)
    if trialstouse(i)
        ...
        if ~isempty(Behavior.Behavior_Frames{i}.states.reward) && Behavior.Behavior_Frames{i}.states.reward(1) < imagingframestouse(end)   %%% If a reward was received 
            ...
            if length(signaltouse{1})<= endcue
                break
            end
            ...
            Trial{i}.trialactivity = cell2mat(reshape(cellfun(@(x) x(startcue:endlicking), signaltouse, 'UniformOutput', false), Fluor.NumberofSpines,1));
            Trial{i}.trialbinaryactivity = processed_signaltouse(:,starttrial:endlicking);
            Trial{i}.trialbinarydendsubactivity = processed_dendsub_signaltouse(:,starttrial:endlicking);
            ...
            Trial{i}.cueactivity = processed_signaltouse(startcue:endcue);
            ...
            Trial{i}.allmovementduringtrialactivity = processed_signaltouse(:,starttrial:endtrial).*repmat(binary_behavior(starttrial:endtrial)', Fluor.NumberofSpines,1);
            Trial{i}.movementduringcueactivity = processed_signaltouse(:,startcue:endcue).*repmat(binary_behavior(startcue:endcue)', Fluor.NumberofSpines,1);
            Trial{i}.successactivity{reward} = processed_signaltouse(startcue:endlicking);
        ...
        elseif isempty(Behavior.Behavior_Frames{i}.states.reward)
            ...
            Trial{i}.trialactivity = cell2mat(reshape(cellfun(@(x) x(starttrial:endtrial), signaltouse, 'UniformOutput', false), Fluor.NumberofSpines,1));
            Trial{i}.trialbinaryactivity = processed_signaltouse(:,starttrial:endtrial);
            Trial{i}.trialbinarydendsubactivity = processed_dendsub_signaltouse(:,starttrial:endtrial);
            ...
            Trial{i}.failureactivity{punish} = processed_signaltouse(:,starttrial:endtrial);
        end
    else
