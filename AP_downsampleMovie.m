...
%    fprintf('%c%c%2d',8,8,round(100*curr_tiff/length(tiff_filenames)));
    waitbar(curr_tiff/length(tiff_filenames), h1, [num2str(round(100*curr_tiff/length(tiff_filenames))), '% of image file info loaded'])
end
disp('Done')
M = all_info{1}.Height;
N = all_info{1}.Width;
...
for curr_summed_frame = 1:summed_frames
    ...
    TifLink = Tiff(im_filenames{curr_file}, 'r');
    for curr_channel = 1:channels
        curr_summed_im = zeros(N,M,'uint16');
        for curr_frame = 1:sum_frames
            TifLink.setDirectory(curr_frame);
            curr_file = file_grp{curr_summed_frame}(curr_file_frame);
...
%    fprintf('%c%c%2d',8,8,round(100*curr_tiff/length(tiff_filenames)));
    waitbar(curr_summed_frame/summed_frames, h1, [num2str(round(100*curr_summed_frame/summed_frames)), '% of image file info loaded'])
end
