function AP_check_motion_correction(date,local_copy,manual_threshold)
...
if ~local_comp
    data_path = ['/usr/local/lab/People/Andy/Data/' date];
    save_filename = ['/usr/local/lab/People/Andy/Data/motion_correction_check' filesep date '_motion_correction_check'];
else
%     data_path = ['Z:\People\Andy\Data\' animal];
    data_path = ['Z:\Data\ImagingRig3\', date, '\NH\', date];
    save_filename = ['Z:\People\Andy\Data\motion_correction_check' filesep date '_motion_correction_check'];
end
        ...
        for curr_tiff = bad_files
            ...
            if ~local_comp
                uncorrected_filename = ['/usr/local/lab/Data/ImagingRig3/' ...
                    uncorrected_file(1:6) filesep date filesep uncorrected_file '.tif'];
            else
...
disp(['Finished checking motion correction for ' date]);


%% For any days that were re-corrected: re-make summed movie
AP_fixed_motion_correction_summed_movies(date,local_copy)
