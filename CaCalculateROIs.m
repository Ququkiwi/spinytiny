...
if ~isempty(existing_ROI)
    ...
    for i = 1:length(existing_ROI)
        ...
        x_c{i} = ROI_pos{i}(1)+ROI_pos{i}(3)/2;                      %%%%% x center being the x coordinate + the width of the ROI (going from edge to edge)
        y_c{i} = ROI_pos{i}(2)+ROI_pos{i}(4)/2;                      %%%%% y center being the y coordinate + the width of the ROI (going from edge to edge)
        theta = (0:1/20:1)*2*pi;
        ...
        ROIreg{i} = roipoly(xsize,ysize, x1{i}, y1{i});
        ROIreg{i} = find(ROIreg{i}(:));
    end
...
for i = 1:length(Fluorescence_Measurement)
    a = medfilt1(Fluorescence_Measurement{i}(baselineFrames)); %%% Perform three successive rounds of median filtering and removing peaks to find the true baseline
    [peaks loc] = findpeaks(a);
    a(loc) = NaN;
    a = a(~isnan(a));
    b = medfilt1(a);
    [peaks loc] = findpeaks(b);
    b(loc) = NaN;
    b = b(~isnan(b));
    c = medfilt1(b);
    [peaks loc] = findpeaks(c);
    c(loc) = NaN;
    c = c(~isnan(c));
    spine_baseline(1,i) = median(c);
end
  
for i = 1:DendNum
    a = medfilt1(Mean_Dend(i,baselineFrames));
    [peaks loc] = findpeaks(a);
    a(loc) = NaN;
    a = a(~isnan(a));
    b = medfilt1(a);
